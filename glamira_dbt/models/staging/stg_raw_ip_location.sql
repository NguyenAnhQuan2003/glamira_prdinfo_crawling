SELECT
  DISTINCT FARM_FINGERPRINT(CONCAT(COALESCE(TRIM(ip), ''), COALESCE(TRIM(city), ''), COALESCE(TRIM(region), ''))) AS location_key,
  COALESCE(NULLIF(TRIM(city), ''), 'Unknown') AS city,
  COALESCE(NULLIF(TRIM(region), ''), 'Unknown') AS region,
  COALESCE(NULLIF(TRIM(country_short), ''), 'Unknown') AS country_short,
  COALESCE(NULLIF(TRIM(country_long), ''), 'Unknown') AS country_long,
